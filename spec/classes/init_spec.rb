  ...
  context 'Supported OS - ' do
    ['Debian', 'RedHat'].each do |osfamily|
      ...
      describe "#{osfamily} configuration via custom template" do
        let(:params) { {
          :config_file_template     => 'collectd/spec.conf',
          :config_file_options_hash => { 'opt_a' => 'value_a' },
        } }
        let(:facts) {{
          :osfamily => osfamily,
        }}
        it { should contain_file('collectd.conf').with_content(/This is a template used only for rspec tests/) }
      ...
      describe "#{osfamily} configuration via custom content" do
        ...
        it { should contain_file('collectd.conf').with_content(/my_content/) }
      end

      describe "#{osfamily} configuration via custom source file" do
        let(:params) { {
          :config_file_source => "puppet:///modules/collectd/spec.conf",
        } }
        let(:facts) {{
          :osfamily => osfamily,
        }}
        it { should contain_file('collectd.conf').with_source('puppet:///modules/collectd/spec.conf') }
      end

      describe "#{osfamily} configuration via custom source dir" do
        let(:params) { {
          :config_dir_source => 'puppet:///modules/collectd/tests/',
      ...
      describe "#{osfamily} service restart disabling on config file change" do
        ...
        it 'should automatically restart the service when files change' do
          should contain_file('collectd.conf').without_notify
        end
      end
